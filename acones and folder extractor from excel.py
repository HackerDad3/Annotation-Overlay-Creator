...
input_file = input("Paste the filepath for the Excel file with link references: ").strip().strip('"')
...
# Convert references to a consistent case (e.g., uppercase) for case-insensitive processing
df['Folder Reference'] = df['Folder Reference'].astype(str).str.upper()
df['Aconex Reference'] = df['Aconex Reference'].astype(str).str.upper()
...
# Deduplicate rows based on Folder Reference, Aconex Reference, and Link
df = df.drop_duplicates(subset=['Folder Reference', 'Aconex Reference', 'Link'])

# Group by 'Folder Reference' and concatenate unique 'Link' values
...
# Group by 'Aconex Reference' and concatenate unique 'Link' values
